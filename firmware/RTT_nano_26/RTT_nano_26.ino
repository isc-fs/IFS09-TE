/* RTT_nano_26.ino — Telemetry Receiver Firmware for IFS09-TE-TeleReceiver
 *
 * Board: IFS09 Custom Telemetry Receiver Board (ATmega328P on Arduino Nano 3.0 USB-C)
 * RF Front-End: nRF24L01+ PA+LNA (with external LM1117 3.3V LDO regulator)
 * Team: ISC Formula Student Racing Team (Season 2026/2027)
 * Author: Andrés Sánchez de Ágreda
 *
 * === HARDWARE PIN MAPPING (Schematics-2.0) ===
 *   CE_PIN           : D7 (PD7 / Pin 10)  — Transceiver Chip Enable
 *   CSN_PIN          : D8 (PB0 / Pin 11)  — SPI Chip Select Not
 *   MOSI_PIN         : D11 (PB3 / Pin 14) — Hardware SPI MOSI
 *   MISO_PIN         : D12 (PB4 / Pin 15) — Hardware SPI MISO
 *   SCK_PIN          : D13 (PB5 / Pin 16) — Hardware SPI Clock
 *   PIN_LED_COMM     : D2 (PD2 / Pin 5)   — LED D1 (Comm Status Indicator / RX activity)
 *   PIN_LED_DETECTED : D3 (PD3 / Pin 6)   — LED D2 (nRF24 Hardware Detected)
 *
 ...
 *   PA level : MAX + LNA Gain Enabled (RF24_PA_MAX, true) for max sensitivity (-95dBm)
 *   AutoAck  : disabled
 *   DynPD    : disabled
 *
 * VERBOSE 0: binary frames only  (use when connected to ISCmetrics host / serial.py)
 * VERBOSE 1: binary + human log  (use when monitoring with a serial terminal / PuTTY)
 */
...
// ------------- Hardware Pin Configuration (IFS09-TeleReceiver) -------------
static const uint8_t PIN_CE           = 7;  // D7 -> nRF24 CE (Pin 3)
static const uint8_t PIN_CSN          = 8;  // D8 -> nRF24 CSN (Pin 4)
static const uint8_t PIN_LED_COMM     = 2;  // D2 -> Comm Status LED (D1)
static const uint8_t PIN_LED_DETECTED = 3;  // D3 -> nRF24 Detected LED (D2)
...
unsigned long comm_led_off_time = 0;
...
void setup() {
    ...
    pinMode(PIN_LED_DETECTED, OUTPUT);
    digitalWrite(PIN_LED_COMM, LOW);
    digitalWrite(PIN_LED_DETECTED, LOW);
    ...
    if (radio_ok && radio.isChipConnected()) {
        // Turn ON nRF24 detected LED to visually indicate SPI hardware connection is verified
        digitalWrite(PIN_LED_DETECTED, HIGH);
        ...
        // Enable maximum power amplifier output & explicitly enable LNA High Gain mode
        // For PA+LNA modules, this biases the RF front-end for maximum RX sensitivity (-95dBm)
...
#if VERBOSE
        Serial.println(F("[RADIO] nRF24L01+ PA+LNA OK — ch76, 1Mbps, CRC8, PA_MAX + LNA Enabled, addr=ECU01"));
        Serial.println(F("[RADIO] Pins: CE=D7, CSN=D8, LED_COMM=D2, LED_DETECTED=D3"));
        ...
        digitalWrite(PIN_LED_DETECTED, LOW);
#if VERBOSE
        Serial.println(F("[RADIO] ERROR — nRF24L01+ not found! Check SPI wiring (CE=D7, CSN=D8)."));
#endif
    }
    last_rx_time = millis();
}
...
void loop() {
    ...
    // Update nRF24 hardware detection LED
    digitalWrite(PIN_LED_DETECTED, chip_connected ? HIGH : LOW);
    ...
    // Handle Communication Status LED timeout (turn off after 40ms pulse)
    if (comm_led_off_time > 0 && now >= comm_led_off_time) {
        digitalWrite(PIN_LED_COMM, LOW);
        comm_led_off_time = 0;
    }
    ...
    if (chip_connected && radio.available()) {
        while (radio.available()) {
            ...
            // Flash Comm Status LED on successful packet reception (active 40ms blink pulse)
            digitalWrite(PIN_LED_COMM, HIGH);
            comm_led_off_time = now + 40;
